...
class SampleSoftmax(Layer):
    ...
    def call(self, x, mask=None, **kwargs):
        ...
        def sampled_softmax(x):
            ...
            batch_loss = K.tf.nn.sampled_softmax_loss(
                self.softmax_w if self.tied_to is None else self.tied_to.weights[0], self.softmax_b,
                next_token_ids_batch, lstm_outputs_batch, num_classes=self.num_classes, num_sampled=self.num_sampled,
                partition_strategy='div')
            batch_loss = K.tf.reduce_mean(batch_loss)
            return [batch_loss, batch_loss]

        def softmax(x):
            lstm_outputs_batch, next_token_ids_batch = x
            logits = K.tf.matmul(lstm_outputs_batch,
                                 K.tf.transpose(self.softmax_w) if self.tied_to is None else K.tf.transpose(
                                     self.tied_to.weight[0]))
