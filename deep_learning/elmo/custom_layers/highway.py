...
class HighWay(layers.Layer):
    ...
    def __init__(self, activation='relu', transform_activation='sigmoid', kernel_initializer='glorot_uniform',
                 transform_initializer='glorot_uniform', bias_initializer='zeros', transform_bias_initializer=-2,
                 kernel_regularizer=None, transform_regularizer=None, bias_regularizer=None,
        ...
        if isinstance(transform_bias_initializer, int):
            self.transform_bias_initializer = initializers.Constant(value=transform_bias_initializer)
        else:
    ...
    def build(self, input_shape):
        ...
        self.w = self.add_weight(shape=(input_dim, input_dim), name='{}_w'.format(self.name),
                                 initializer=self.kernel_initializer, regularizer=self.kernel_regularizer,
                                 constraint=self.kernel_constraint)
        self.w_transform = self.add_weight(shape=(input_dim, input_dim), name='{}_w_transform'.format(self.name),
                                           initializer=self.transform_initializer,
                                           regularizer=self.transform_regularizer,
                                           constraint=self.kernel_constraint)
        self.bias = self.add_weight(shape=(input_dim,), name='{}_bias'.format(self.name),
                                    initializer=self.bias_initializer, regularizer=self.bias_regularizer,
                                    constraint=self.bias_constraint)
        self.bias_transform = self.add_weight(shape=(input_dim,), name='{}_bias_transform',
