...
class MultiHeadAttention(nn.Module):
    def __init__(self, hidden_size, dropout_rate, head_size=8):
        ...
        self.linear_q = nn.Linear(hidden_size, head_size * attn_size, bias=True)
        self.linear_k = nn.Linear(hidden_size, head_size * attn_size, bias=True)
        self.linear_v = nn.Linear(hidden_size, head_size * attn_size, bias=True)
...
class Encoder(nn.Module):
    def __init__(self, hidden_size, filter_size, dropout_rate, n_layer):
        super(Encoder, self).__init__()

        encoders = [EncoderLayer(hidden_size, filter_size, dropout_rate) for _ in range(n_layer)]
...
class Decoder(nn.Module):
    def __init__(self, hidden_size, filter_size, dropout_rate, n_layer):
        ...
        decoders = [DecoderLayer(hidden_size, filter_size, dropout_rate) for _ in range(n_layer)]
...
class Transformer(nn.Module):
    def __init__(self, i_vocab_size, t_vocab_size, n_layer=6, hidden_size=512, filter_size=2048, dropout_rate=0.1,
                 share_target_embedding=True, has_inputs=True, src_pad_idx=None, trg_pad_idx=None):
        ...
        self.decoder = Decoder(hidden_size, filter_size, dropout_rate, n_layer)
        ...
        if self.has_inputs:
            ...
            self.encoder = Encoder(hidden_size, filter_size, dropout_rate, n_layer)
    ...
    def encode(self, inputs, i_mask):
        ...
        input_embedded.masked_fill_(i_mask.sequeeze(1).unsequeeze(-1), 0)
        input_embedded *= self.emb_scale
        input_embedded += self.get_position_encoding(input_embedded)
    ...
    def decode(self, targets, enc_output, i_mask, t_self_mask, t_mask, cache=None):
        ...
        target_embedded.masked_fill_(t_mask.sequeeze(1).unsequeeze(-1), 0)
